...
void setUpInitializationValues(MrfIo *impl, const Mac802154Config *config)
{
  ...
  MrfIo_setControlRegister_Expect(impl, mrf_register_power_amplifier_control2, mrf_fifo_enable
  ...
  MrfIo_setControlRegister_Expect(impl, mrf_register_rf_control6,
  ...
  MrfIo_setControlRegister_Expect(impl, mrf_register_sleep_clock_control1,
...
void test_sendBlocking(void)
{
  ...
  MrfField full_header = {.data = fake_header_data, .length = fake_header_length, .address = fake_header_memory_address,};
  MrfState_getFullHeaderField_ExpectAnyArgsAndReturn(full_header);

  MrfIo_writeBlockingToLongAddress_Expect(NULL, full_header.data, full_header.length, full_header.address);
  MrfIo_writeBlockingToLongAddress_IgnoreArg_mrf();

  MrfField payload_field = {.data = (uint8_t *) payload, .length = payload_length, .address = fake_header_length,};
  MrfState_getPayloadField_ExpectAnyArgsAndReturn(payload_field);

  MrfIo_writeBlockingToLongAddress_Expect(NULL, payload_field.data, payload_field.length, payload_field.address);
  MrfIo_writeBlockingToLongAddress_IgnoreArg_mrf();
...
void test_getReceivedFramePayload(void)
{
  ...
  FrameHeader802154 *frame = (FrameHeader802154 *) (random_packet_data + frame_length_field_size);
  ...
  TEST_ASSERT_EQUAL_PTR(random_packet_data + header_size + frame_length_field_size, payload);
}
...
void test_getPacketExtendedSourceAddress(void)
{
  ...
  TEST_ASSERT_EQUAL_HEX8_ARRAY(address, Mac802154_getPacketExtendedSourceAddress(mrf, packet), 8);
}
...
test_getPacketShortSourceAddress(void)
{
  ...
  TEST_ASSERT_EQUAL_HEX8_ARRAY(address, Mac802154_getPacketShortSourceAddress(mrf, packet), 2);
}
...
void test_enablePromiscuousMode(void)
{
  Mrf *impl = (Mrf *) mrf;
  MrfIo_setControlRegister_Expect(&impl->io, mrf_register_receive_mac_control, 1);
...
{
  Mrf *impl = (Mrf *) mrf;
  MrfIo_setControlRegister_Expect(&impl->io, mrf_register_receive_mac_control, 0);
